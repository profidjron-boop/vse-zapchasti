...
export default function AdminReportsPage() {
      ...
      try {
        ...
        const parseSection = async <T,>(
          ...
          try {
            ...
            if (redirectIfAdminUnauthorized(sectionError, router)) {
              throw sectionError;
            }
            if (
              sectionError instanceof ApiRequestError &&
              sectionError.traceId
            ) {
              failedSections.push(
                `${section.label} (Код: ${sectionError.traceId})`,
              );
            } else {
        ...
        if (failedSections.length > 0) {
          setError(
            `Часть разделов не загрузилась: ${failedSections.join(", ")}`,
          );
        }
        setLastUpdated(new Date().toLocaleTimeString("ru-RU"));
      } catch (fetchError) {
        if (
          redirectIfAdminUnauthorized(fetchError, router)
        ) {
          return;
        }
        setError(toAdminErrorMessage(fetchError, "Ошибка загрузки отчётов"));
      } finally {
        setLoading(false);
        setRefreshing(false);
      }
  ...
  const vinStatusRows = useMemo(
    () => groupStatusCounts(vinRequests),
    [vinRequests],
  );
  const serviceStatusRows = useMemo(
    () => groupStatusCounts(serviceRequests),
    [serviceRequests],
  );
      ...
      <div className="mb-6 flex flex-wrap items-center justify-between gap-3">
        <div>
          ...
          <p className="mt-1 text-sm text-neutral-600">
            Минимальная операционная сводка по каталогу и заявкам.
          </p>
        </div>
