...
export default function CartPage() {
  ...
  const goToCatalogLabel = contentValue(
    "cart_go_to_catalog_label",
    "Перейти в каталог",
  );
  const checkoutTitle = contentValue(
    "cart_checkout_title",
    "Оформление заказа",
  );
  ...
  async function handleSubmitOrder(event: FormEvent<HTMLFormElement>) {
    ...
    const selectedPaymentMethod = (formData.get("payment_method")?.toString() ||
      "") as "cash_on_delivery" | "invoice";
    const legalEntityInn =
      formData.get("legal_entity_inn")?.toString().trim() || "";
    ...
    try {
      ...
      const invoiceFile =
        invoiceRequisitesFile instanceof File && invoiceRequisitesFile.size > 0
          ? invoiceRequisitesFile
          : null;
      ...
      const payload = {
        source: "checkout",
        customer_name:
          formData.get("customer_name")?.toString().trim() || undefined,
        customer_phone: customerPhone,
        customer_email:
          formData.get("customer_email")?.toString().trim() || undefined,
        comment: formData.get("comment")?.toString().trim() || undefined,
        delivery_method:
          formData.get("delivery_method")?.toString() || undefined,
        payment_method: formData.get("payment_method")?.toString() || undefined,
        legal_entity_name:
          formData.get("legal_entity_name")?.toString().trim() || undefined,
        ...
        items: items.map((item) => ({
          ...
          line_total:
            item.price !== null ? item.price * item.quantity : undefined,
        })),
      };
              ...
              <div className="mt-8 grid gap-3 sm:grid-cols-2">
                <div className="rounded-2xl border border-white/10 bg-white/8 px-5 py-4">
                  ...
                  <div className="mt-2 text-base font-semibold">
                    Откройте каталог и добавьте нужные позиции
                  </div>
                </div>
                <div className="rounded-2xl border border-white/10 bg-white/8 px-5 py-4">
                  <div className="text-xs uppercase tracking-[0.18em] text-white/60">
                  ...
                  <div className="mt-2 text-base font-semibold">
                    Оформите заказ или отправьте заявку на подбор
                  </div>
                </div>
              </div>
            ) : (
          ...
          <div className="rounded-[2rem] border border-neutral-200 bg-white p-10 text-center shadow-[0_18px_44px_rgba(15,23,42,0.05)]">
            <div className="mx-auto max-w-2xl">
              ...
              <p className="mt-3 text-sm leading-7 text-neutral-600">
                Начните с каталога или VIN-подбора. Когда добавите товары, здесь
                появится полный checkout с доставкой, оплатой и заявкой
                менеджеру.
              </p>
                    ...
                    >
                      <div className="flex flex-col gap-4 sm:flex-row sm:items-start sm:justify-between">
                        <div className="min-w-0">
                          ...
                          <h3 className="mt-2 text-lg font-semibold text-[#10264B]">
                            {item.name}
                          </h3>
                          <div className="mt-3 flex flex-wrap gap-2 text-sm text-neutral-600">
                            <span className="rounded-full bg-neutral-100 px-3 py-1">
                              {item.price !== null
                                ? `${Math.round(item.price).toLocaleString("ru-RU")} ₽/шт.`
                                : "Цена по запросу"}
                            </span>
                            <span className="rounded-full bg-neutral-100 px-3 py-1">
                              Итого: {getLineAmount(item)}
                            </span>
                          </div>
                        </div>
                ...
                <div className="mt-4 min-h-[5rem]">
                  ...
                  {!error && success ? (
                    ...
                    >
                      ...
                      <Link
                        href="/account/orders"
                        className="mt-2 inline-block text-xs font-semibold text-green-800 hover:underline"
                      >
                    ...
                    <div className="grid gap-3 sm:grid-cols-2">
                      ...
                      <div className="sm:col-span-2">
                        ...
                        <p className="mt-2 text-xs leading-5 text-neutral-500">
                          Необязательно. Можно приложить карточку организации
                          или PDF с реквизитами. Допустимы PDF, PNG и JPG до 10
                          МБ.
                        </p>
                        {selectedInvoiceFileName ? (
                          <p className="mt-2 text-xs font-medium text-[#10264B]">
                            Выбран файл: {selectedInvoiceFileName}
                          </p>
                        ) : null}
                      </div>
                    </div>
