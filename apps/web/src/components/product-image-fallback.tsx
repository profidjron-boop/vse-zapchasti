...
export function ProductImageFallback({
  ...
  return (
    ...
    >
      ...
      <div className={`relative flex h-full w-full flex-col justify-between ${compact ? "p-3" : "p-5"}`}>
        <div className="flex items-center justify-between gap-2">
          <span className={`font-semibold tracking-[0.18em] text-white/75 ${compact ? "text-[9px]" : "text-[11px]"}`}>
            VSE ZAPCHASTI
          </span>
          <span className={`rounded-full border border-white/20 bg-white/10 text-white/85 ${compact ? "px-2 py-0.5 text-[9px]" : "px-2.5 py-1 text-[10px]"}`}>
            {brand?.trim() || "PARTS"}
          </span>
        </div>

        <div className="space-y-2">
          <div className={`max-w-[12rem] font-semibold leading-tight ${compact ? "text-xs" : "text-lg"}`}>
            {title}
          </div>
          <div className={`text-white/70 ${compact ? "text-[10px]" : "text-xs"}`}>
            Артикул: {sku}
          </div>
        </div>
      </div>
