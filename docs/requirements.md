...
- Для оплаты по счёту допускается необязательное вложение файла с реквизитами организации (PDF/JPG/PNG, self-hosted upload).
...
2. Режимы обновления каталога зафиксированы как `manual | hourly | daily | event`:
   ...
   - для API-native event trigger поддержан `POST /api/admin/products/import-from-source`.
